...
export function findIntersection(ellipse, line) {
  ...
  if (k === Infinity) {
    ...
    // 检查交点是否在直线段内
    if (
      x1_intersection >= Math.min(x1, x2) &&
      x1_intersection <= Math.max(x1, x2)
    ) {
      intersections.push({ x: x1_intersection, y: y1_intersection });
    }
    if (
      x2_intersection >= Math.min(x1, x2) &&
      x2_intersection <= Math.max(x1, x2)
    ) {
...
export function getDrawerData(
  ...
  if (currentInfo.selectType === "ellipse") {
    ...
    tools = {
      ...
      width: Math.min(
        drawInfo.width - 10,
        Math.max(Math.abs(currentInfo.line.x2 - currentInfo.line.x1), 20)
      ),
    };
  }
            ...
            ...(options?.isTest ? [
              ...
              },
              {
                ...
                style: {
                  ...
                  x: 0,
                  y: 0,
                  text: `${options.linear.expression}`,
                },
              },
            ] : []),
          ],
